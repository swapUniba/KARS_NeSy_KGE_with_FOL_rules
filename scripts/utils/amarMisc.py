...
def top_scores(predictions, n):
  ...
  for u in list(set(predictions['users'])):
    topk_user = predictions.loc[predictions['users'] == u ].head(n)
    top_n_scores = pd.concat([top_n_scores, topk_user])
  return top_n_scores

def read_ratings(df: pd.DataFrame):
  return list(df[0]), list(df[1]), list(df[2])
